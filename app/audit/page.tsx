...
export default function AuditPage() {
  ...
  const getActionColor = (action: string) => {
    switch (action) {
      case 'created':
        return 'bg-brand-green-500/20 text-emerald-200 border-emerald-400/50';
      case 'deleted':
        return 'bg-brand-pink-500/20 text-red-200 border-red-400/50';
      case 'updated':
        return 'bg-brand-blue-500/20 text-blue-200 border-blue-400/50';
      default:
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        ...
        {loading ? (
          <div className="glass-effect p-8 rounded-lg shadow-md border border-brand-gray/30 text-center">
            <p className="text-brand-black">Laden...</p>
          </div>
        ) : logs.length === 0 ? (
          <div className="glass-effect p-8 rounded-lg shadow-md border border-brand-gray/30 text-center">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {logs.map((log) => (
                  ...
                  >
                    <td className="px-3 py-2 text-brand-black/90">{formatDate(log.timestamp)}</td>
                    ...
                    <td className="px-3 py-2 font-medium text-brand-black">{log.studentName}</td>
                    ...
                    <td className="px-3 py-2">
                      {log.reverted ? (
                        ...
                        <span className="px-2 py-1 rounded text-xs bg-brand-green-500/30 text-emerald-300">
                          Actief
                        </span>
                      )}
                    </td>
                        ...
                        <button
                          ...
                          className={`px-3 py-1 text-xs rounded font-medium transition-colors ${
                            revertingId === log.id
                              ...
                              : 'bg-yellow-600 text-brand-black hover:bg-yellow-700'
                          }`}
                        >
